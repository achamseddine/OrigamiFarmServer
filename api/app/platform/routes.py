...
def issue_licence(
    ...
    # Recorded because it matters who could have known the password, not
    # just that one was set.
    if pack.owner_password is not None:
        record_audit_event(
            db,
            actor_id=identity.user_id,
            actor_type=ActorType.PLATFORM_USER,
            tenant_id=tenant_id,
            action="tenant_user.password_set_by_admin",
            entity_type="user_identity",
            entity_id=str(owner.id),
            summary=f"Set a password for {owner.email} while issuing a licence",
        )
...
@router.post(
    "/tenants/{tenant_id}/memberships/{membership_id}/password",
    response_model=MemberPasswordOut,
)
def set_member_password_endpoint(
    tenant_id: uuid.UUID,
    membership_id: uuid.UUID,
    payload: MemberPasswordRequest,
    db: Session = Depends(get_control_db),
    identity: Identity = Depends(require_platform_role(*_STAFF_AND_SUPPORT)),
) -> MemberPasswordOut:
    """Sets a farm user's password directly, and shows it once.

    The answer for a deployment with no mail server: rather than a link
    somebody has to receive, an admin sets the password and reads it to the
    customer. Generated by default, from an alphabet with no confusable
    characters, because it gets dictated down a phone line.

    It is recorded as a password the holder did not choose, so the console
    keeps saying so until they replace it themselves from the tablet app
    (POST /api/v1/auth/change-password) — setting one for somebody is only
    defensible if they can take it back.
    """
    membership = _get_membership_or_404(db, tenant_id, membership_id)
    user = db.get(UserIdentity, membership.user_id)
    if user is None:
    ...
    password = set_member_password(db, user=user, password=payload.new_password)
    ...
    record_audit_event(
        ...
        summary=f"Set a password for {user.email}",
    )

    return MemberPasswordOut(
        email=user.email,
        display_name=user.display_name,
        password=password,
        must_change=True,
