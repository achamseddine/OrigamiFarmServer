...
| UI-only permission gating | A person's permissions are re-read server-side on every protected route from their own membership grid — the tablet's copy of `/me/access` is for rendering only, never a boundary. (Per-*module* gating is gone with device licences: one subscription covers the whole product, so there is nothing left to gate.) | `app/farmos/deps.py:require_permission` |
...
| Revoked device continuing to operate | Device status checked first, before membership/tenant lookups, in `get_tenant_context`; a revoked device stays revoked however often somebody signs in on it | `app/auth/dependencies.py`, `app/farmos/routes_auth.py:_record_device` |
...
| Unaudited privileged actions | Every tenant status change, repricing, device revoke, membership grant, admin-set password, support session, export request writes an `audit_event` **in the same DB transaction** as the change | `app/audit/service.py`, called throughout `app/platform`, `app/entitlements/state_machine.py` |
| Secrets in git | `.env` files and `infrastructure/keys/*.pem` are gitignored; `.env.example` ships placeholders only | `.gitignore`, `.env.example` |
